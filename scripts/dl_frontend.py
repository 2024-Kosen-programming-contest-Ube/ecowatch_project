...
def get_urls_from_assets(assets):
    ...
    try:
        for asset in assets:
            if asset["name"] == "ecowatch_classroom.zip":
...
def download(file_url):
    ...
    if res.status_code == 200:
        ...
        shutil.move(f"{common.BASE_PATH}/tmp/{unziped}",
                    f"/var/www/html/{unziped}")
        common.remove_exists(filename)


def download_frontend():
    res = requests.get(
        "https://api.github.com/repos/2024-Kosen-programming-contest-Ube/ecowatch/releases/latest")
    if res.status_code != 200:
        return
    data = res.json()
    # print(data)
